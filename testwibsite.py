...
class MainPage(BasePage):
    ...
    def click_about_us(self):
        self.page.wait_for_selector(
            self.ABOUT_US, state="visible", timeout=120000
        )  # Увеличено до 120 секунд
        self.page.click(self.ABOUT_US)

    def click_services(self):
        self.page.wait_for_selector(
            self.SERVICES, state="visible", timeout=120000
        )  # Увеличено до 120 секунд
        self.page.click(self.SERVICES)

    def click_projects(self):
        self.page.wait_for_selector(
            self.PROJECTS, state="visible", timeout=120000
        )  # Увеличено до 120 секунд
        self.page.click(self.PROJECTS)

    def click_reviews(self):
        self.page.wait_for_selector(
            self.REVIEWS, state="visible", timeout=120000
        )  # Увеличено до 120 секунд
        self.page.click(self.REVIEWS)

    def click_contacts(self):
        self.page.wait_for_selector(
            self.CONTACTS, state="visible", timeout=120000
        )  # Увеличено до 120 секунд
...
def test_navigation(page, link, expected_url):
    ...
    page.wait_for_url(
        expected_url, timeout=120000, wait_until="networkidle"
